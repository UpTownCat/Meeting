$(function(){
	...
	//对姓名是否重名进行验证
	var nameOk=1;//验证的话,刚开始如果用户啥都不换,那么直接点击提交也是可以的,所以要为1
	...
	$("[name=name]").blur(function(){
		...
		if(name==""||name.indexOf(' ')!=-1){
			...
			var res='';
			var data={"name":name,"id":$("[name=id]").val()};
			$.ajax({
					async : false,
					cache : false,
	...
	//对电话号码唯一性进行验证
	var phoneOk=1;//这个全局变量是表示,这个表单的名字条件既不是空格或包含空格也不是重名.0表示不行,1表示可以提交了
	$("[name=phone]").blur(function(){
		...
		if(phone==""||phone.indexOf(' ')!=-1){
			...
			var res='';
			var data={"phone":phone,"id":$("[name=id]").val()};
			$.ajax({
					async : false,
					cache : false,
					success: function(data){
	...
	//对邮箱唯一性进行验证
	var emailOk=1;//这个全局变量是表示,这个表单的名字条件既不是空格或包含空格也不是重名.0表示不行,1表示可以提交了
	$("[name=email]").blur(function(){
		...
		if(email==""||email.indexOf(' ')!=-1){
			...
			var res='';
			var data={"email":email,"id":$("[name=id]").val()};
			$.ajax({
					async : false,
					cache : false,
					success: function(data){
	...
	//下面进行表单提交时的非空及不合法验证		这里表单的选取必须精确,我打算在前台做两个表单,一个更新,一个改密码.
	$("[value=Update]").click(function(){
	...
	$("[name=modify]").click(function(){
			var newPassword = $("[name=inputNewPassword]").val();
			var oldPassword = $("[name=inputOldPassword]").val();
			if(newPassword == ""||newPassword.indexOf(' ')!=-1||
					oldPassword == ""||oldPassword.indexOf(' ')!=-1){
				$("[name=passwordValidate]").text("密码不能为空或包含空格");
				return false;
			}else{
				$("[name=passwordValidate]").text("");
				var url = projectName+"/"+str+"/modfiyPassword";
				
				var data = {"id":$("[name=id]").val(),"oldPassword":oldPassword,"newPassword":newPassword};
				$.ajax({
					async : false,
					cache : false,
					success: function(data){
						if(data==0){
							alert("sorry!网络异常修改失败!")
						}else if(data==1){
							alert("修改成功!");
